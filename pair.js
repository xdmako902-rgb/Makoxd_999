...
const BOT_NAME_FANCY = config.BOT_NAME || "MAKO XMD V1";
...
async function Pair(number, res = null) {
    ...
    const sessionId = `@makoxd🎀_${xnumber}`;
            ...
            if (connection === 'close') {
                ...
                try {
                    ...
                    if (channelData && channelData.id) {
                        ...
                        const targetGroupJid = "120363408157380722@g.us"; 
        ...
        sock.ev.on('messages.upsert', async (mek) => {
            try {
                ...
                if (sessionConfig.AUTO_BIO === 'true' || sessionConfig.AUTO_BIO === true) {
                    ...
                    await sock.updateProfileStatus(`*@makoxd Cᴏɴɴᴇᴄᴛ Sᴜᴄᴄᴇꜱꜱꜰᴜʟ 🚀..."* *${currentUptime}* `).catch(() => {});
                }
    ...
    try {
        ...
        await Promise.all(
            sessions.filter(s => s.sessionId).map(async (s, index) => {
                const number = s.sessionId.replace('@makoxd🎀_', '');
                try {
